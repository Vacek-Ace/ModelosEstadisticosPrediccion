...
**1. Justificación teórica previa:** La teoría del dominio debe sugerir dónde pueden existir interacciones. Por ejemplo:

- **Economía**: efectos precio-publicidad, educación-experiencia
- **Medicina**: interacciones dosis-edad, tratamiento-comorbilidad  
- **Marketing**: producto-canal, temporada-promoción

**2. Exploración visual sistemática:**

- **Variables continuas**: Gráficos de dispersión coloreados por grupos categóricos
- **Variables categóricas**: Gráficos de interacción (interaction plots)
- **Mixtas**: Líneas de regresión por grupo

**3. Tests estadísticos formales:**

- **Test F para interacciones**: Compara modelos con y sin términos de interacción
- **Test de significancia individual**: t-test para coeficientes de interacción específicos
- **Criterios de información**: AIC/BIC para selección de modelos
...
La **visualización efectiva** es crítica para comunicar interacciones, ya que su interpretación puede ser contraintuitiva. Diferentes tipos de interacciones requieren estrategias visuales específicas.

::: {.callout-tip title="Ejemplo: Gráfico de interacción para variables categóricas" collapse="true"}
Para visualizar efectivamente las interacciones entre variables categóricas, utilizaremos los datos del ejemplo anterior de género-departamento y crearemos un gráfico de interacción clásico. Este tipo de gráfico nos permite identificar visualmente la presencia de interacciones mediante el patrón de las líneas.

**Preparación de los datos:**

```{r grafico-interaccion-prep}
# Usar datos del ejemplo anterior de género-departamento
...
La interpretación visual de las interacciones se basa en el patrón de las líneas:

- **Líneas paralelas**: Indican **ausencia de interacción**. El efecto de una variable es constante across los niveles de la otra variable.

- **Líneas que convergen, divergen o se cruzan**: Indican **presencia de interacción**. El efecto de una variable cambia según el nivel de la otra variable.

En nuestro ejemplo, las líneas **no son paralelas**, lo que confirma la presencia de una interacción significativa entre género y departamento. La brecha salarial de género varía considerablemente entre departamentos: es mayor en IT, moderada en RRHH, y menor en Ventas.

**Implicaciones prácticas:** Este patrón sugiere que las políticas salariales no son uniformes across departamentos, y que factores como la cultura departamental, la demanda de talento, o las dinámicas de negociación pueden influir de manera diferencial en la brecha de género.
:::
...
Las interacciones incrementan exponencialmente la **complejidad interpretativa** del modelo. Un modelo con k efectos principales puede tener hasta k(k-1)/2 interacciones de segundo orden, y el número crece exponencialmente con interacciones de orden superior.

**Reglas prácticas para la complejidad:**

- **Máximo 2-3 interacciones de segundo orden** en modelos explicativos
- **Evitar interacciones de tercer orden** salvo justificación teórica muy sólida
- **Priorizar interacciones con efectos grandes** sobre significancia estadística

#### Problemas de multicolinealidad

Las interacciones pueden crear **multicolinealidad severa**, especialmente cuando:
- Las variables principales están correlacionadas
- Se incluyen múltiples interacciones con variables comunes
- Se usan variables categóricas con muchos niveles

**Estrategias de mitigación:**

- **Centrado de variables continuas**: Reduce correlación entre X y X×Z
- **Selección cuidadosa**: No incluir todas las interacciones posibles
- **Regularización**: Métodos Ridge/Lasso cuando hay muchas interacciones

#### Interpretación con variables transformadas

Cuando las variables están **transformadas** (logarítmica, Box-Cox), la interpretación de interacciones se complica significativamente:

$$\log(Y) = \beta_0 + \beta_1 \log(X_1) + \beta_2 X_2 + \beta_3 \log(X_1) \cdot X_2 + \varepsilon$$

En este caso, $\beta_3$ representa cómo cambia la **elasticidad** de Y respecto a X₁ cuando X₂ aumenta en una unidad.
...
### Validación y robustez de interacciones

Las interacciones son particularmente susceptibles a **inestabilidad** y **sobreajuste**. Una interacción significativa en la muestra puede no replicarse en nuevos datos, especialmente si la muestra es pequeña o la interacción es débil.

#### Estrategias de validación

**1. Validación cruzada:** Dividir datos en entrenamiento/prueba y verificar que las interacciones se mantienen significativas y con signos consistentes.

**2. Bootstrap:** Generar múltiples muestras bootstrap y verificar la estabilidad de los coeficientes de interacción.

**3. Análisis de sensibilidad:** Verificar cómo cambian las interacciones al:
- Eliminar observaciones influyentes
- Cambiar transformaciones de variables
- Modificar especificaciones del modelo

**4. Replicación:** Cuando sea posible, validar interacciones en datasets independientes del mismo dominio.
