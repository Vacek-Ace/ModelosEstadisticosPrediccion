...
- Reduce magnitud de coeficientes, no los elimina
- Maneja multicolinealidad
- No realiza selección de variables

**Lasso:** Penalización $L_1 = \lambda \sum |\beta_j|$
- Puede anular coeficientes completamente
- Selección automática de variables
- Problemas con variables altamente correlacionadas

**Elastic Net:** Combina $L_1 + L_2$
- Mayor flexibilidad con correlación entre predictores
...
- Ridge reduce magnitud de coeficientes pero no los anula
- **Lasso puede eliminar variables por completo** (coeficientes = 0)

**Interpretación del parámetro $\lambda$:**
- $\lambda = 0$: equivalente a regresión lineal tradicional
- $\lambda$ aumenta: más coeficientes se reducen a cero
- $\lambda$ muy grande: se eliminan demasiadas variables
...
**Fundamento:** Técnica de regularización que combina las propiedades de Ridge y Lasso
...
**Ventajas sobre métodos individuales:**
- Manejo superior de multicolinealidad
- Selección de variables más estable
- Evita selección arbitraria cuando hay grupos de variables correlacionadas
...
- **Ridge Regression**
  - Penalización: $L_2: \sum \beta_j^2$
  - Efecto: Reduce magnitud de coeficientes
  - Selección de variables: No

- **Lasso (Least Absolute Shrinkage)**
  - Penalización: $L_1: \sum |\beta_j|$
  - Efecto: Puede anular coeficientes completamente
  - Selección de variables: Sí

- **Elastic Net**
  - Penalización: Combinación $L_1 + L_2$
  - Efecto: Combina ambos efectos
  - Selección de variables: Sí

**Cuándo usar cada método:**
- **Ridge:** Todas las variables aportan información, hay multicolinealidad
- **Lasso:** Muchas variables irrelevantes, selección sparse
- **Elastic Net:** Variables correlacionadas, modelo estable y parsimonioso
...
- **$\lambda_{min}$:** Valor que minimiza el error de CV
- **$\lambda_{1SE}$:** Valor más grande cuyo error está dentro de 1 error estándar del mínimo
...
**La pregunta fundamental:** ¿Funcionará bien nuestro modelo con datos que nunca ha visto?

**El problema del sobreajuste:**

Un modelo puede "memorizar" los datos de entrenamiento, incluyendo:
- Ruido aleatorio específico de la muestra
- Patrones espurios que no se generalizan
- Peculiaridades que no representan la población

**Analogía:** Como estudiar solo exámenes pasados para un examen nuevo

**La validación simula este "examen final":**
- Evalúa la capacidad de **generalización**
- Proporciona estimación **honesta** del rendimiento predictivo
...
- **Error LOOCV:** $CV_{(n)} = \frac{1}{n}\sum_{i=1}^{n}(y_i - \hat{y}_i)^2$
...
**Para regresión lineal:** Fórmula rápida disponible usando leverage
...
# Advertencias Importantes sobre los Métodos Automáticos
...
**Mejor práctica:** Usar como paso inicial, pero validar con validación cruzada rigurosa

# Resumen: El Proceso Completo de Selección y Validación
...
**Mensaje final:** El mejor modelo es aquel que resuelve el problema específico con la mayor simplicidad e interpretabilidad posible.
...
**Ventajas:**
- **Manejo de multicolinealidad:** Distribuye penalización entre variables correlacionadas
- **Selección más estable:** Evita selección arbitraria de Lasso
- **Mejora rendimiento predictivo:** Optimización mediante validación cruzada

**Estrategias para $\alpha$:**
- Variables irrelevantes → $\alpha$ cercano a 1 (Lasso)
- Fuerte multicolinealidad → $\alpha$ cercano a 0 (Ridge)
- Balance selección/estabilidad → $\alpha = 0.5$

# Validación del Modelo

**Pregunta fundamental:** ¿Cómo estar seguros de que el modelo funcionará bien con datos nuevos?

**El problema del sobreajuste:**
- Modelo "memoriza" datos de entrenamiento
- Incluye ruido y peculiaridades específicas
- Pierde capacidad de **generalizar** a nuevas observaciones

**La validación simula el "examen final":**
- Estimar rendimiento predictivo en el mundo real
- **Estrategias de validación:** cómo simular el examen
- **Métricas de evaluación:** cómo calificarlo

**Principio clave:** Un buen ajuste en datos de entrenamiento NO garantiza buen rendimiento predictivo

# Estrategias de Validación

**Partición inicial (paso no negociable):**
1. **Datos de modelado (80%):** Para construcción y evaluación de modelos
2. **Conjunto de prueba final (20%):** Guardado bajo llave para evaluación final

**Train/Test Split (dentro de datos de modelado):**
- **Conjunto de entrenamiento (70-80%):** El modelo aprende
- **Conjunto de test (20-30%):** Evaluación del rendimiento
- **Problema:** Resultados dependen de la división aleatoria específica

**Validación cruzada k-fold:**
- Dividir datos en $k$ particiones
- Usar cada partición como test una vez
- Promediar resultados de las $k$ iteraciones
- **Error de validación cruzada:** $CV_{(k)} = \frac{1}{k}\sum_{i=1}^{k} \text{Métrica}_i$
...
**Validación cruzada k-fold:**
- Dataset tamaño moderado (n < 1000)
...
**LOOCV (Leave-One-Out):**
- Dataset pequeño (n < 100)
- Estimación menos sesgada posible
- Tiempo computacional no es crítico
- En regresión lineal: fórmula rápida disponible
...
- **Trata todos los errores proporcionalmente**
- Más robusto frente a valores atípicos
- Interpretación directa: "error promedio"
- Más fácil de interpretar para muchos

# Interpretando el Error

**Comparación Error Entrenamiento vs Error Test:**

**Sobreajuste (Overfitting):**
- Error entrenamiento **bajo** + Error test **mucho más alto**
- Modelo ha "memorizado" el ruido
- **Solución:** Simplificar modelo, usar regularización

**Subajuste (Underfitting):**
- Error entrenamiento **alto** + Error test **alto y similar**
- Modelo demasiado simple
- **Solución:** Aumentar complejidad, añadir variables/interacciones

**Modelo bien ajustado:**
- Error entrenamiento y test **similares y bajos**
- Buen equilibrio entre sesgo y varianza

**Regla general:** Error entrenamiento siempre más bajo que error test. La clave está en analizar la **diferencia** entre ambos.

# Resumen y Conclusiones

**Proceso sistemático completo:**

1. **Filtrado inicial** → Eliminar variables problemáticas
2. **Criterios de información** → Comparar modelos objetivamente  
3. **Métodos de selección** → Exhaustivos vs automáticos vs regularización
4. **Validación rigurosa** → Evaluar capacidad de generalización

**Principios clave:**
- **No existe método perfecto** → Combinar enfoques
- **Validación es indispensable** → Datos no vistos
- **Simplicidad cuando es posible** → Parsimonia
- **Conocimiento del dominio** → Siempre considerarlo

**El mejor modelo es aquel que resuelve el problema específico con la mayor simplicidad posible**
