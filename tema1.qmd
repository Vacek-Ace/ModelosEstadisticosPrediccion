...
para la predicción y la inferencia [@draper1998applied].
...
en su alcance y en los retos que presentan[@weisberg2005applied].
...
la calidad del ajuste del modelo y la adecuación del modelo a los
...
hipótesis, es decir, proceder al diagnóstico del modelo. El análisis de
los residuos nos permitirá identificar deficiencias en la verificación
de estas hipótesis, ası́ como la detección de observaciones anómalas o
influyentes.

Recordemos que los residuos del modelo son las diferencias entre los
valores observados y los valores predichos por el modelo:

$$
e_i = Y_i - \hat{Y}_i.
$$ Los residuos representan la parte de la variabilidad en $Y$ no
explicada por el modelo.

En ocasiones, es preferible trabajar con los **residuos
estandarizados**, que tienen media cero y varianza, aproximadamente,
uno: $$
d_i = \frac{e_i}{\sqrt{MSE}},  \hspace{0.5cm} i=1,\ldots,n.
$$ Otro tipo de residuos habitual son los llamados **residuos
estudentizados**: $$
...
A continuación se ejecutarán pruebas estadísticas (contrastes de
hipótesis) para confirmar los hallazgos visuales.
...
Un gráfico de residuos frente a valores ajustados ayuda a identificar si
...
residuos frente a los valores ajustados se observa un patrón de "embudo" o una variación creciente/disminuyente, es decir, cuando se observa que la
...
-   QQ-Plot (Gráfico Cuantil-Cuantil): Compara la distribución de los
    ...
    normal estándar, $\phi^{−1}_{[(i − 1)/n]}$. Si es cierta la
    normalidad de los residuos, el gráfico resultante deberı́a
...
donde $\text{SE}(\hat{\beta}_j)$ es el error estándar del coeficiente.
Este estadístico sigue una distribución $t_{n-p-1}$.
