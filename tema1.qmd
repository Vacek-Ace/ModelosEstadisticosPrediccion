...
regresión lineal múltiple, presentado como unageneralización del simple,
y para abordar las complejidades adicionales que surgen, como la
...
respuesta, o lo que es lo mismo, del error aleatorio: 4. **Normalidad:**
Los términos de error $\varepsilon$ siguen una distribución normal con
...
$$
\text{$$E} = \sum_{i=1}^n e_i^2 =\sum_{i=1}^{n} (Y_i - \hat{Y}_i)^2 = \sum_{i=1}^{n} \left(Y_i - (\hat{\beta_0} + \hat{\beta_1} X_i)\right)^2.
$$

::: {.callout-note title="Minimización de $$E" collapse="true"}
...
residuos ($$$E$). Aquí está el proceso paso a paso:

Para minimizar $$$E$, derivamos parcialmente con respecto a $\beta_0$ y
$\beta_1$ y resolvemos el sistema de ecuaciones.

1.  **Primera derivada con respecto a** $\beta_0$: $$
    \frac{\partial $$E}{\partial \beta_0} = -2 \sum_{i=1}^n \left(Y_i - (\beta_0 + \beta_1 X_i)\right).
    $$ Igualando a cero: $$
...
2.  **Primera derivada con respecto a** $\beta_1$: $$
    \frac{\partial $$E}{\partial \beta_1} = -2 \sum_{i=1}^n X_i \left(Y_i - (\beta_0 + \beta_1 X_i)\right).
    $$ Igualando a cero: $$
...
-   Las varianzas de los estimadores de $\beta_0$ y $\beta_1$ son: $$
    ...
    \hat{\sigma}^2=\frac{$$E}{n-2},
    $$ siendo $$$E=\sum_{i=1}^{n}(Y_i-\hat{Y_i})^2$, la suma de los
...
$$\
donde $\bar{Y}$ es la media de $Y$. $$$T$ refleja la dispersión general
de $Y$ respecto a su media.

**Descomposición de la variabilidad**

El modelo de regresión lineal permite descomponer$$$T$ en dos
...
-   $$$R$ (Suma de los Cuadrados del Modelo): Representa la variabilidad
    explicada por la regresión, es decir, la parte de $Y$ que se puede
    predecir a partir de $X$:\
    $$
      $$R = \sum_{i=1}^n (\hat{Y}_i - \bar{Y})^2,
     $$ donde $\hat{Y}_i$ es el valor predicho por el modelo para el
    $i$-ésimo dato.

-   $$$E$ (Suma de los Cuadrados de los Errores): Como hemos visto al
...
($df$), suma de cuadrados ($$$$), media cuadrática ($MS$) y el
estadístico $F$:

| Fuente    | $df$  | $$$$  | $MS = $$/df$      | Estadístico$F$ |
|-----------|-------|-------|-------------------|----------------|
| Regresión | 1     | $$$R$ | $MSR = $$R/1$     | $F = MSR/MSE$  |
| Error     | $n-2$ | $$$E$ | $MSE = $$E/(n-2)$ |                |
| Total     | $n-1$ | $$$T$ |                   |                |
...
$$
R^2 = 1 - \frac{\text{Suma de los Cuadrados de los Residuos ($$E)}}{\text{Suma Total de los Cuadrados ($$T)}} = \frac{\text{Suma de los Cuadrados del Modelo ($$R)}}{\text{Suma Total de los Cuadrados ($$T)}}.
$$

Donde:

-   $\text{$$T} = \sum_{i=1}^n (Y_i - \bar{Y})^2$: Variabilidad total en
    $Y$.\
-   $\text{$$R} = \sum_{i=1}^n (\hat{Y}_i - \bar{Y})^2$: Variabilidad
    explicada por el modelo.\
-   $\text{$$E} = \sum_{i=1}^n (Y_i - \hat{Y}_i)^2$: Variabilidad no
    explicada.
...
-   $R^2$ debe ser interpretado con cautela, ya que resultará con
    ...
    lineal.
-   Ası́ por ejemplo, la magnitud d \$R\^2\$2 depende del rango de
...
Recordemos que los residuos del modelo ($e_i$) son las diferencias entre
los valores observados ($Y_i$) y los valores predichos ($\hat{Y}_i$) por
el modelo: $$
e_i = Y_i - \hat{Y}_i.
...
-   QQ-Plot (Gráfico Cuantil-Cuantil): Compara la distribución de los
    ...
    ordenados $e[i]$ frente a los cuantiles correspondientes de una
    normal estándar, $\phi^{−1}[(i − 1)/n]$. Si es cierta la normalidad
...
La varianza de los errores ($\sigma^2$) mide cuánto varían las observaciones reales ($Y_i$) con respecto a los valores predichos por el modelo ($\hat{Y}_i$). En el modelo de regresión múltiple, esta varianza puede ser estimada utilizando la **suma de los cuadrados de los residuos ($$E)**.


La estimación de $\sigma^2$, denotada como $\hat{\sigma}^2$, se calcula como:
$$
\hat{\sigma}^2 = \frac{\text{$$E}}{n - p - 1},
$$
donde:

  - $\text{$$E} = \sum_{i=1}^n (Y_i - \hat{Y}_i)^2 = \mathbf{e}^\top \mathbf{e}$ es la suma de los cuadrados de los residuos.
...
Para construir intervalos de confianza para los coeficientes de la regresión, 
$\beta_0, \beta_1, \ldots, \beta_k$, procedemos de forma análoga a como 
hacíamos en el caso de la regresión lineal simple. Usamos la distribución en 
el muestreo de los estimadores, $\hat{\beta}$:
$$
\hat{\beta} \sim N_p(\beta, \sigma^2 (\mathbf{X}^\top \mathbf{X})^{-1}).
$$

Esto implica que cada coeficiente $\hat{\beta}_j$ es normal con media 
$\beta_j$ y varianza $\sigma^2 C_{jj}$, con $C_{jj}$ el elemento $j$-ésimo 
de la diagonal de la matriz $(\mathbf{X}^\top \mathbf{X})^{-1}$.
...
1. **Suma de Cuadrados del Modelo de Regresión (SSR):** Variabilidad explicada por los regresores.
2. **Suma de Cuadrados de los Residuos (SSE):** Variabilidad no explicada por el modelo.
