...
En este tema, abordaremos los fundamentos de la regresión lineal
...
regresión lineal múltiple, presentado como una generalización del simple,
...
$$
\text{$$E} = \sum*{i=1}\^n e_i\^2 =*\sum{i=1}\^{n} (Y_i - \hat{Y}*i)\^2
=* \sum{i=1}\^{n} \left(Y_i - (\hat{\beta_0} + \hat{\beta_1}
X_i)\right)\^2. \$\$
...
Para minimizar \$\$$E$, derivamos parcialmente con respecto a $\beta_0$
y $\beta_1$ y resolvemos el sistema de ecuaciones.

1.  **Primera derivada con respecto a** $\beta_0$: $$
    \frac{\partial $$E}{\partial \beta*0} = -2* \sum{i=1}\^n \left(Y_i -
    (\beta\_0 + \beta*1 X_i)*\right). $$ Igualando a cero: $$
    \sum{i=1}\^n \left(Y_i - \beta\_0 - \beta\_1 X_i\right) = 0. \$\$
...
2.  **Primera derivada con respecto a** $\beta_1$: $$
    \frac{\partial $$E}{\partial \beta*1} = -2* \sum{i=1}\^n X_i
    \left(Y_i - (\beta\_0 + \beta*1 X_i)*\right).
    $$ Igualando a cero: $$ \sum{i=1}\^n X_i \left(Y_i - \beta\_0 -
    \beta\_1 X_i\right) = 0. \$\$
...
$$\
donde $\bar{Y}$ es la media de $Y$. \$\$$T$ refleja la dispersión
general de $Y$ respecto a su media.
...
$$
R^2 = 1 - \frac{\text{Suma de los Cuadrados de los Residuos ($$E)}}{\text{Suma Total de los Cuadrados ($$T)}}
=
\frac{\text{Suma de los Cuadrados del Modelo ($$R)}}{\text{Suma Total de los Cuadrados ($$T)}}.
\$\$
...
como: $$
\hat{\sigma}^2 = \frac{\text{$$E}}{n - p - 1}, \$\$ donde:

-   $\text{$$E} = \sum_{i=1}^n (Y_i - \hat{Y}_i)^2 = \mathbf{e}^\top \mathbf{e}$
...
significativo), - $H_1:$ al menos uno de los $\beta_j \neq 0$.
