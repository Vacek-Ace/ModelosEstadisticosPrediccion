...
**En un escenario ideal**, el gráfico debería parecer una nube de
...
- Para el modelo correcto: p-valor = `r round(reset_resultado$p.value, 4)` → No
rechazamos H₀ (forma funcional correcta) 
- Para el modelo no lineal:
p-valor = `r round(reset_no_lineal$p.value, 4)` → Rechazamos H₀ (forma
...
El estadístico de Durbin-Watson varía entre 0 y 4. Un valor cercano a 2 sugiere no
...
- **Durbin-Watson**: DW =`r round(dw_resultado_valido$statistic, 3)`, p-valor = `r round(dw_resultado_valido$p.value, 4)` → No hay autocorrelación de orden 1 
- **Breusch-Godfrey**: LM =`r round(bg_resultado$statistic, 3)`, p-valor = `r round(bg_resultado$p.value, 4)` → No hay autocorrelación de orden 2 fluctúan aleatoriamente alrededor del cero. 

La prueba de Durbin-Watson arroja un estadístico de **`r round(dw_resultado_valido$statistic, 3)`**
(cercano a 2) y un **p-valor de `r format.pval(dw_resultado_valido$p.value, digits=3, eps=0.001)`**,
...
**✅ Homocedasticidad**: La prueba de Breusch-Pagan arroja un p-valor de `r round(bp_test$p.value, 4)`, que es mayor a 0.05, por lo que no hay evidencia de heterocedasticidad. La varianza de los errores es constante.

**✅ Normalidad**: La prueba de Shapiro-Wilk presenta un p-valor de `r round(sw_test$p.value, 4)`, superior a 0.05, confirmando que los residuos siguen una distribución normal. Esto se corrobora visualmente en el Q-Q plot.

**✅ Independencia**: El estadístico de Durbin-Watson es `r round(dw_test$statistic, 3)` con un p-valor de `r format.pval(dw_test$p.value, digits=3, eps=0.001)`, indicando ausencia de autocorrelación en los residuos.

**✅ Media nula**: La media de los residuos es `r round(media_residuos, 10)`, prácticamente cero como se esperaría en un modelo bien especificado.
