...
$\varepsilon_i$ se comportan de una manera predecible y ordenada. Estos supuestos, conocidos como condiciones de Gauss-Markov [@kutner2005applied; @weisberg2005applied], son fundamentales para las propiedades óptimas de los estimadores de mínimos cuadrados.
...
residuos ($SSE$). Este método, desarrollado por Legendre y Gauss a principios del siglo XIX [@galton1886; @weisberg2005applied], es fundamental en la estadística moderna. Aquí está el proceso paso a paso:
...
**diagnóstico del modelo** [@fox2018r; @harrell2015]. Este proceso consiste en verificar si se
cumplen los supuestos del modelo de regresión lineal clásico. La
fiabilidad de nuestras inferencias (los p-valores de los contrastes t y
F, y los intervalos de confianza) depende directamente de la validez de
estos supuestos.
...
La herramienta fundamental para diagnosticar la linealidad es el gráfico
de **residuos** ($e_i$) frente a los **valores ajustados** por el modelo
($\hat{y}_i$). La lógica de este gráfico es sencilla pero potente: si el
...
la magnitud de las predicciones. En esencia, buscamos confirmar que no
queda ninguna información sistemática en los errores que el modelo no
haya capturado.

**En un escenario ideal**, este gráfico debería parecer una nube de
...
se conoce como **heteroscedasticidad**, y es un problema común en el
modelado.
...
es estable a lo largo de los valores ajustados. Esto se confirma con la
prueba de Breusch-Pagan, que arroja un **p-valor alto**, por lo que no
tenemos evidencia para rechazar la hipótesis nula de homocedasticidad.
**Nuestro modelo cumple el supuesto**.
:::
...
Para evaluar la normalidad disponemos de herramientas visuales y
analíticas:
...
la curva normal teórica (línea roja). La prueba de Shapiro-Wilk confirma
esto con un **p-valor alto**
(`r format(shapiro_resultado$p.value, scientific = TRUE, digits = 3)`),
por lo que no rechazamos la hipótesis nula de normalidad.
:::
...
La **Prueba de Durbin-Watson** es el contraste clásico para la
autocorrelación de primer orden. Su estadístico se calcula como:
$$ DW = \frac{\sum_{i=2}^{n}(e_i - e_{i-1})^2}{\sum_{i=1}^{n}e_i^2} $$
El estadístico varía entre 0 y 4. Un valor cercano a 2 sugiere no
autocorrelación. Valores cercanos a 0 indican autocorrelación positiva,
y cercanos a 4, autocorrelación negativa.
...
muestra ningún patrón sistemático o tendencia temporal, los puntos
fluctúan aleatoriamente alrededor del cero. La prueba de Durbin-Watson
arroja un estadístico de **`r round(dw_resultado_valido$statistic, 3)`**
(cercano a 2) y un **p-valor de
...
aspectos complementarios de la influencia [@kutner2005applied; @fox2018r]. Las métricas desarrolladas por Cook, Belsley, Kuh y Welsch proporcionan herramientas robustas para este diagnóstico.
...
El umbral de influencia para DFFITS es `r round(dffits_threshold, 3)`. En nuestro modelo, **`r length(influential_dffits)` observaciones superan este umbral**: las observaciones `r paste(influential_dffits, collapse = ", ")`, lo que las clasifica como influyentes según este criterio.

Las **cinco observaciones con mayor |DFFITS|** son las observaciones `r paste(top_indices, collapse = ", ")`, con valores de `r paste(round(dffits_vals[top_indices], 3), collapse = ", ")` respectivamente. Lo más notable es que **todas estas cinco observaciones** (`r paste(intersect(top_indices, influential_dffits), collapse = ", ")`) **superan el umbral de DFFITS**, confirmando su carácter influyente.

**Interpretación clave:** La observación `r top_indices[1]` es el caso más destacado: tiene un DFFITS de `r round(dffits_vals[top_indices[1]], 3)`, superando el umbral de `r round(dffits_threshold, 3)`. Esta combinación de residuo y apalancamiento resulta en un DFFITS significativo que indica cambios sustanciales en su predicción.

**Conclusión práctica:** Tenemos **`r length(influential_dffits)` observaciones influyentes** según DFFITS (`r paste(influential_dffits, collapse = ", ")`) que merecen investigación adicional. Estas observaciones cambian significativamente sus propias predicciones cuando son eliminadas del modelo, sugiriendo que podrían representar casos especiales o errores de medición que deberían ser examinados más detalladamente.
:::
...
El gráfico revela varios puntos importantes. Tenemos `r length(outliers_stud)` outliers (residuos estudentizados > 2): las observaciones `r paste(outliers_stud, collapse = ", ")`. Además, `r length(high_leverage)` observaciones superan el umbral de leverage (> `r round(leverage_threshold, 3)`): las observaciones `r paste(high_leverage, collapse = ", ")`.

**Interpretación por regiones:**

- **Zona derecha (alto leverage)**: Las observaciones `r paste(high_leverage, collapse = ", ")` superan el umbral de leverage, lo que significa que tienen valores de X atípicos y **alto potencial influyente**
- **Zona izquierda superior/inferior**: Los `r length(outliers_stud)` outliers (`r paste(outliers_stud, collapse = ", ")`) están distribuidos aquí, con leverage bajo-moderado pero residuos grandes
...
**Conclusión práctica:** El modelo presenta una **situación favorable**: aunque tenemos **outliers** (observaciones `r paste(outliers_stud, collapse = ", ")`) que son atípicos en Y, y **observaciones de alto leverage** (observaciones `r paste(high_leverage, collapse = ", ")`) que son atípicos en X, **crucialmente no hay solapamiento entre ambos grupos**. Esto significa que no tenemos la situación más problemática (alto leverage + outlier). Aun así, ambos grupos merecen investigación.
:::
...
✅ **Linealidad**: Sin patrones en residuos vs ajustados - ✅
**Homocedasticidad**: Varianza constante (Breusch-Pagan p \> 0.05) - ✅
**Normalidad**: Los residuos siguen distribución normal (Shapiro-Wilk p
\> 0.05) - ✅ **Independencia**: Sin autocorrelación (Durbin-Watson ≈
2) - ✅ **Media nula**: La media de residuos es prácticamente 0 - ✅
**Sin valores influyentes**: No hay observaciones problemáticas según
Cook, DFFITS o COVRATIO - ✅ **Sin outliers**: No hay residuos
estandarizados extremos

Esto confirma que nuestras inferencias estadísticas (p-valores,
intervalos de confianza) son válidas y confiables [@james2021; @harrell2015].
:::